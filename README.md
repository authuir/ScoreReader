...
The `online-library/public/` folder is deployed as a static site:

```
online-library/public/
├── scores/        # YOUR *.mxl / *.musicxml / *.xml uploads (tracked in git)
├── library.json   # auto-regenerated by build_site.py (gitignored)
└── index.html     # auto-regenerated by build_site.py (gitignored)
```
...
2. Upload `.mxl` files into `online-library/public/scores/` — either by
   committing locally or by using the GitHub web UI (**Add file → Upload
   files** while inside that folder).
3. The Pages workflow runs automatically and publishes to
   `https://<user>.github.io/<repo>/library.json`.
4. In the app, open **Settings → Online library URL** and paste that URL.
